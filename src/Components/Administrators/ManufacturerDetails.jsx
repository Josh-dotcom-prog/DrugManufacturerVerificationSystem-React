...
const ManufacturerDetails = () => {
    ...
    useEffect(() => {
        async function fetchManufacturer() {
            try {
                const response = await api.get(`/admin/user?manufacturer_id=${id}`);
    ...
    const handleApprove = async () => {
        try {
            const response = await api.patch(`/admin/approve?manufacturer_id=${id}`);
    ...
    if (!token) {
        return <div className="p-6 text-red-600">You are not authenticated. Please log in.</div>;
    }
    const viewCertificate = async () => {
        try {
            const token = localStorage.getItem("access_token"); // or however you store it

            const response = await api.get(`/admin/certificate?manufacturer_id=${manufacturer.id}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
                responseType: 'blob', // This is KEY for PDF
            });

            const file = new Blob([response.data], { type: 'application/pdf' });
            const fileURL = URL.createObjectURL(file);
            window.open(fileURL); // browser now knows it's a PDF
        } catch (error) {
    ...
    const handleReject = async () => {
        ...
        try {
            await api.patch(`/admin/reject?manufacturer_id=${id}`, { reason: rejectionReason }); // Adjust endpoint
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-900">{manufacturer.phone_number}</p>
                                </div>
                                <div>
